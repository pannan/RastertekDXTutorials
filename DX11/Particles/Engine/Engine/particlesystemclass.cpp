...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		while(!found)
		{
			//因为在kill particles时已经移动过粒子，就是n个粒子，哪里数组中前n个元素都是active的粒子，没有空隙
			//active == false表示插入粒子的z是最靠近相机的，因为前面m_currentParticleCount个active粒子的positionZ < positionZ判断都是失败
			//或者positionZ < positionZ表示插入粒子需要在中间插入
		...
		//当上面判断active == false，说明插入粒子最靠近相机，index == m_currentParticleCount
		//下面的while里的代码不会执行
		//否则
		//需要把z值比插入粒子更靠近相机的粒子忘后移动一位，为新插入粒子空出位置
		while(i != index)
		{
